...
1. Choose `TypeVariable(K)` with `READY_FOR_FIXATION_LOWER`
...
1. Choose `TypeVariable(K)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
2. __NotEnoughInformationForTypeParameter__
3. `TypeVariable(K) == ERROR CLASS: Cannot infer argument for type parameter K` _from Fix variable K_
...
1. Choose `TypeVariable(X)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
...
1. Choose `TypeVariable(T)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
...
2. Choose `TypeVariable(X)` with `READY_FOR_FIXATION_LOWER`
...
6. Choose `TypeVariable(T)` with `READY_FOR_FIXATION_LOWER`
...
1. `kotlin/Unit <: kotlin/Unit` _from LambdaArgument_
2. Choose `TypeVariable(X)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
    1. `TypeVariable(T)` is `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
...
5. Choose `TypeVariable(T)` with `WITHOUT_PROPER_ARGUMENT_CONSTRAINT`
